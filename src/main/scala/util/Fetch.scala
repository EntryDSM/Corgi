...
object Fetch {
  def authenticatedRequest(context: Context, method: Method, urls: String*): Request = {
    ...
    val requestingURL = URL.join(registryURL +: urls: _*)
    val userID = context.userID
    val password = context.password
    val requestObject = Request(method, requestingURL).basicAuth(userID, password)
